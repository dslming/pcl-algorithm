...
class BST {
    ...
    static delete(value) {
        ...
        if (!node.left && !node.right) {
            if (!node.parent) {
                tree = null
            } else if (node.parent.left && node.parent.left.value == value) {
        ...
        if (node.left && node.right) {
            ...
            if (node.parent && 
                node.parent.left && 
                node.parent.left.value == value) {
                node.parent.left = replace
            } else if(node.parent && 
                node.parent.right && 
                node.parent.right.value == value) {
            ...
            // 遍历所有重新渲染
            BST.breadthFirstTraverse(replace,item=>{
                item.extras.initialized = false
                item.extras.entered = false
            })
        }
...
window.addEventListener("mouseup", ({ clientX, clientY }) => {
    ...
    if (!inserting && dragDistance < 2) {
        ((async () => {
            inserting = true;
            tree = await BST.push(tree, Math.round(Math.random() * 10));
            inserting = false;
            window.tree = tree
        })())
    }
